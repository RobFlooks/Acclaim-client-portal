...
- **Scheduled Reports**: **ADMIN-ONLY CONFIGURATION** - Only administrators can configure scheduled email reports for users. Users cannot see or modify their own report settings. Admins configure reports via the Users tab in the Admin Panel by clicking on the Calendar/CalendarOff icon to open a configuration dialog. Features include: daily, weekly or monthly frequency selection, time of day selection (any hour from 12:00 AM to 11:00 PM), day of week (for weekly) or day of month (for monthly) scheduling, Case Summary tab (case name, account number, debtor, status, amounts), Activity Report tab (messages received), case status filter (active only, all cases, or closed only). Admins can also send test reports to verify settings. Reports are sent as Excel and PDF attachments via email. The scheduler runs hourly to process due reports. **ADMIN ORGANISATION CONTROLS** - Admins can enable/disable scheduled reports per organisation via the Organisations tab (Calendar/CalendarOff toggle). When disabled for an organisation, report processing filters out cases/messages from disabled organisations. The Users tab shows each user's scheduled report status: green Calendar icon with frequency for enabled, grey CalendarOff with frequency for disabled, or just CalendarOff icon for not configured.
