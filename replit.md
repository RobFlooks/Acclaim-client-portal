...
- **Admin Controls**: Email domain restrictions for admin privileges (@chadlaw.co.uk only). **SUPER ADMIN SYSTEM** - Sensitive admin functions are restricted to designated "super admins" defined in the `SUPER_ADMIN_EMAILS` environment variable (comma-separated list). Super admin-only functions include: deleting users, granting/removing admin privileges, viewing/managing system audit logs, managing scheduled reports (create/update/delete/test), and deleting messages/documents. Regular admins can still view users, organisations, and perform non-destructive actions. The API provides `/api/admin/super-admin-status` endpoint for the UI to check access levels.
